...
queueSize := 0
indLastRefreshed := 0
OutputDebug, This queueSize=%queueSize%
...
{
        global 
        OutputDebug, Enqueue(%searchLocal%) - %queueSize%
        queue[queueSize] := searchLocal
        ; queue.Push(%searchLocal%)
        OutputDebug % "length=" . queue.Length()
        OutputDebug % "queue[" . queueSize . "] = " . queue[queueSize]
        queueSize := queueSize + 1
        OutputDebug, QueueSize=%queueSize%
}

RefreshTimer:
        global indLastRefreshed
        global queue
        global queueSize
        if (indLastRefreshed < queueSize - 1)
        {
                OutputDebug, RefreshTimer - %indLastRefreshed% - %queueSize%
                s := queue[indLastRefreshed]
                OutputDebug, q=%s%
                indLastRefreshed := queueSize - 1
                ; RefreshWindowList(queue[indLastRefreshed])
        }
        ; SetTimer, RefreshTimer, -100
        return
...
{
        OutputDebug, RefreshWindowlist(%searchLocal%)
